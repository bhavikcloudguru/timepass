...
@Component({
  ...
  styleUrls: ['./autocomplete-country.component.scss'],
  providers: [
    { provide: MatFormFieldControl, useExisting: AutocompleteCountryComponent }
  ]
})
export class AutocompleteCountryComponent
   ...
   */
  @HostBinding()
  id = `autocomplete-flag-${AutocompleteCountryComponent.nextId++}`;
  ...
  set required(req) {
    ...
    if (this._required) {
      this.autocompleteFlagForm.controls['autocompleteInput'].setValidators(
        Validators.required
      );
    }
    this.stateChanges.next();
    ...
    if (value !== null) {
      ...
      return this.portList.filter(
        obj =>
          ...
          (obj.countryCode && obj.countryCode.toLowerCase().indexOf(v) === 0) ||
  ...
  private getFilteredOptions(): Observable<OriginDestination[]> {
    return this.autocompleteFlagForm.controls.autocompleteInput.valueChanges.pipe(
      ...
      map(value => {
        ...
        } else if (value && value.countryCode) {
          this.toHighlight = value.countryCode;
          return this.filter(value.countryCode.trim());
        } else {
